...
int main(){
    ...
    mat feil_spess = ones<vec>(n_maks);
    mat feil_gen = ones<vec>(n_maks);
...
if(N<=4){
    ...
    string filnavn = "../Plotting_kjetil/n_";
    filnavn += to_string(N);
    max_error = 0;
    ...
    feil_spess[N-1] = log10(max_error);
...
outfile.open("../../tex_files/figures/tid.tex");
outfile << fixed;
...
//outfile << "Time comparisson 1b, 1c" <<endl;
outfile << "$ \\log_{10}$(h) &   $ \\epsilon_{Max}$ , general $\\epsilon_{Max} $, special\\\\ " << endl;
outfile << "\\hline  " <<endl;


for(int i=0; i<n_maks; i++){
    outfile <<  - (i+1) <<" & "<< feil_gen[i]<<" & "<< feil_spess[i]<< " \\\\ "<<  endl;
}
